...
passport.deserializeUser(async (id, done) => {
  try {
    const user = getQuery('SELECT * FROM users WHERE id = ?', [id]);
...
passport.use(new GoogleStrategy({
  ...
  async (accessToken, refreshToken, profile, done) => {
    try {
      // Upsert user
      const existingUser = getQuery('SELECT * FROM users WHERE id = ?', [profile.id]);
      ...
      const user = getQuery('SELECT * FROM users WHERE id = ?', [profile.id]);
